...
| Lead webhook | Separate database outbox with atomic claim, lease, retry, signed payload, and stable idempotency key. Generic, Slack, and Discord payload shapes are available. Email delivery remains independent. The repository uses a daily Vercel cron compatible with the Hobby plan. | Configure endpoint and secret, run migration, and prove receiver deduplication and retries against the chosen service in staging. Delivery may wait up to 24 hours on the included schedule; use Vercel Pro or an external authenticated scheduler if a shorter interval is required. |
